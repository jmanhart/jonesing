...
// Routes that serves index.html
router.get('/', (request, response) => {
    response.setHeader('Content-Type', 'text/html');

    // Capture the content of index.html in a variable
    let fileContents = fs.readFileSync("./public/index.html", {encoding: "utf8"});

    // Send a repsonce to the client with the index.html file
...
router.post('/', (request, response) => {
    request.on("data", function(inputValue){
        ...
        // Parse the query into a key/value pair and store the value of numberOf 
        // Paragraphs in a variable
